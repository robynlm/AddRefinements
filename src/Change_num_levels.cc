...
void AddRefinement_Change_num_levels(CCTK_ARGUMENTS) {
    ...
    int min_active_n = -1;
    float min_threshold = std::numeric_limits<float>::max();
    ...
    // Determine the minimum active threshold
    for (int n = 0; n < 40; ++n) {
        if (addref_threshold_active[n] && !addref_threshold_done_already[n]) {
            if (addref_threshold[n] < min_threshold) {
                min_threshold = addref_threshold[n];
                min_active_n = n;
            }
        }
    }
    ...
    if (min_active_n != -1) {
      if (cctk_time >= addref_threshold[min_active_n]) {
        ...
        if (reduced_variable >= addref_threshold[min_active_n]) {
          num_levels[0] += 1;
          addref_threshold_done_already[min_active_n] = 1;
          CCTK_ParameterSet("regrid_every", "CarpetRegrid2", "1");
